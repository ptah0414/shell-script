...
# 가상머신 생성 함수
vmcreation(){
        dialog --title "이미지 선택하기" \
		--radiolist "베이스 이미지를 아래에서 선택하세요" 15 50 5 \
        "CentOS7" "센토스 7 베이스 이미지" ON \
        "Ubuntu" "우분투 20.04 베이스 이미지" OFF \
        "RHEL" "레드햇 엔터프라이즈 리눅스 8.0" OFF \
        2> $image
        ...
        *)
        ...
        then
                ...
                then
                        dialog --title "스펙 선택" \
						--radiolist "필요한 자원을 선택하세요" 15 50 5 \
                        "m1.small" "가상 CPU 1개, 메모리 1GB" ON \
                        "m1.medium" "가상 CPU 2개, 메모리 2GB" OFF \
                        "m1.large" "가상 CPU 4개, 메모리 8GB" OFF \
                        2> $flavor
...
vmdeletion() { 
        ...
        for i in $(virsh list --all | grep -v "Name" | gawk '{print $2}' | sed '/^$/d')
        do
            RADIOLIST="$RADIOLIST $i $COUNTER off "
            let COUNTER=COUNTER+1
        done

        dialog --title "삭제할 인스턴스 선택" \
        --radiolist "test" 0 0 $COUNTER \
        $RADIOLIST 2> $deletion

        if [ $? -eq 0 ]
        then 
                dialog --yesno "정말로 삭제하시겠습니까?" 0 0 
                if [ $? -eq 0 ]
                then 
                        todelete=$(cat $deletion)

                        virsh destroy $todelete
                        virsh undefine $todelete --remove-all-storage

                        if [ $? -eq 0 ]
                        then 
                                dialog --msgbox "성공적으로 삭제되었습니다" 0 0
                                vmlist
                        fi 
                else
                        break
...
do
        # 메인메뉴 출력하기
        dialog --menu "KVM 관리 시스템" 20 40 8 \
        1 "가상머신 리스트" \
        2 "가상 네트워크 리스트" \
        3 "가상머신 생성" \
        4 "가상머신 삭제" \
